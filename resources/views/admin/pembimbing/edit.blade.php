<x-layouts.internal>
    <div class="mb-8">
        ...
        <h2 class="text-2xl font-bold text-gray-800 font-sans">Edit Master Pembimbing</h2>
        <p class="text-xs text-gray-400 mt-1">Perbarui data, kuota, atau bidang penugasan untuk {{ $pembimbing->nama }}.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 max-w-3xl">
        <form action="{{ route('admin.pembimbing.update', $pembimbing->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="nama" id="nama" value="{{ old('nama', $pembimbing->nama) }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="nip" id="nip" value="{{ old('nip', $pembimbing->nip) }}"
                        class="w-full text-xs rounded-xl border border-gray-200 px-4 py-2.5 focus:ring-2 focus:ring-biogen-medium focus:border-biogen-medium outline-none transition"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $pembimbing->email) }}"
                        class="w-full text-xs rounded-xl border border-gray-200 px-4 py-2.5 focus:ring-2 focus:ring-biogen-medium focus:border-biogen-medium outline-none transition"
                    ...
                    @foreach($bidangs as $b)
                        <label class="flex items-center space-x-2.5 p-3 rounded-xl border border-gray-200 hover:border-biogen-medium hover:bg-emerald-50/50 cursor-pointer transition">
                            <input type="checkbox" name="bidang_ids[]" value="{{ $b->id }}"
                                {{ in_array($b->id, $assignedBidangIds) ? 'checked' : '' }}
            ...
            <div class="pt-2">
                <label class="flex items-center space-x-2.5 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $pembimbing->is_active) ? 'checked' : '' }}
                        class="rounded border-gray-300 text-biogen-medium focus:ring-biogen-medium">
