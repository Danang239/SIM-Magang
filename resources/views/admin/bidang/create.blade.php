<x-layouts.internal>
    <div class="mb-8">
        ...
        <h2 class="text-2xl font-bold text-gray-800 font-sans">Tambah Bidang Penempatan</h2>
        <p class="text-xs text-gray-400 mt-1">Buat divisi atau laboratorium baru untuk program PKL di BRMP Biogen.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 max-w-4xl">
        <form action="{{ route('admin.bidang.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="nama_bidang" id="nama_bidang" value="{{ old('nama_bidang') }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="jenjang" id="jenjang" required
                        ...
                        <option value="Mahasiswa" {{ old('jenjang') === 'Mahasiswa' ? 'selected' : '' }}>Mahasiswa (D3/D4/S1/S2)</option>
                        <option value="Siswa" {{ old('jenjang') === 'Siswa' ? 'selected' : '' }}>Siswa (SMK/SMA Sederajat)</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="kategori" id="kategori" required
                        class="w-full text-xs rounded-xl border border-gray-200 px-4 py-2.5 focus:ring-2 focus:ring-biogen-medium focus:border-biogen-medium outline-none bg-white transition">
                        <option value="Mahasiswa" {{ old('kategori') === 'Mahasiswa' ? 'selected' : '' }}>Mahasiswa (Perguruan Tinggi)</option>
                        <option value="Siswa" {{ old('kategori') === 'Siswa' ? 'selected' : '' }}>Siswa (SMK / SMA Sederajat)</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3" required
                    ...
                    placeholder="Jelaskan fokus utama kegiatan di laboratorium/divisi ini...">{{ old('deskripsi') }}</textarea>
            </div>
                        ...
                        <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-200 hover:border-biogen-medium bg-gray-50/50 hover:bg-emerald-50/20 transition">
                            <label class="flex items-center space-x-3 cursor-pointer flex-grow">
                                <input type="checkbox" name="pembimbing_ids[]" value="{{ $p->id }}"
                                    {{ in_array($p->id, old('pembimbing_ids', [])) ? 'checked' : '' }}
                            ...
                            <div class="flex items-center space-x-2 shrink-0">
                                ...
                                <input type="number" name="kuota_pembimbing[{{ $p->id }}]"
                                    value="{{ old('kuota_pembimbing.'.$p->id, $p->kuota_default) }}" min="1" max="50"
