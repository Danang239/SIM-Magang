<x-layouts.internal>
    <!-- Header Section -->
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-gray-800 font-sans">Ulasan Laporan Akhir Magang</h2>
        <p class="text-xs text-gray-400 mt-1">Daftar laporan akhir yang telah diunggah oleh peserta magang aktif dan menunggu peninjauan.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 mb-6">
        <form method="GET" action="{{ route('petugas.review-laporan.index') }}" class="flex items-center space-x-3">
            ...
            @if(request('search'))
                <a href="{{ route('petugas.review-laporan.index') }}" class="px-4 py-2.5 bg-gray-50 border border-gray-200 hover:bg-gray-100 text-gray-500 rounded-xl text-xs font-semibold transition-colors">
                    Reset
                </a>
            @endif
        </form>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        @if($pengajuans->isEmpty())
            <div class="text-center py-16">
                ...
                <p class="text-xs text-gray-500 font-medium">Tidak ada laporan akhir magang yang perlu diulas saat ini.</p>
            </div>
        @else
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 text-[10px] font-bold text-gray-400 uppercase tracking-wider border-b border-gray-100">
                            ...
                            <th class="px-6 py-4">Bidang / Pilihan</th>
                            <th class="px-6 py-4">Tanggal Rencana Selesai</th>
                            <th class="px-6 py-4">Ulasan Laporan</th>
                        ...
                        @foreach($pengajuans as $item)
                            <tr class="hover:bg-gray-50/70 transition-colors">
                                ...
                                <td class="px-6 py-4 font-semibold text-gray-850">
                                    {{ $item->user->name }}
                                    <p class="text-[10px] text-gray-400 font-normal mt-0.5">{{ $item->user->instansi ?? '-' }}</p>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-gray-500">
                                    {{ \Carbon\Carbon::parse($item->tanggal_selesai_rencana)->translatedFormat('d M Y') }}
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-block text-[9px] bg-yellow-100 text-yellow-750 font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                                        {{ $item->laporan_status }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('petugas.review-laporan.show', $item->public_id) }}" class="bg-biogen-medium hover:bg-biogen-light text-white text-[10px] font-bold px-4 py-2 rounded-lg shadow-sm transition-all inline-block">
                                        Tinjau Laporan
                                    </a>
                                </td>
                            </tr>
